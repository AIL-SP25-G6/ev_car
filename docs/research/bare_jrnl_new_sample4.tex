...
affecting 41\% of records, systematic brand-level pricing errors, and 54.4\%
...
practical accuracy for budget and mid-range segments comprising 56\% of the
market. Our analysis highlights that data quality remediation contributes
...
\begin{enumerate}
    ...
    segments, which constitute 56\% of the Vietnamese EV market.
\end{enumerate}
...
\midrule
bonbanh.com & BeautifulSoup & Multi-brand & 1,502 \\
chotot.com & Scrapy & Multi-brand & 1,727 \\
xevinfastluot.com & Selenium & VinFast only & 208 \\
otodien.vn & BeautifulSoup & Multi-brand & 729 \\
\midrule
...
Systematic analysis of the merged dataset revealed three critical data
quality issues, each with significant impact on model performance.
...
1,727 records (41.4\% of the dataset) and was traced to a unit mismatch in
the LLM extraction pipeline where prices in thousands of VND were
interpreted as raw VND values.
...
All 74 BYD-brand records (2.3\% of the dataset) exhibited prices
...
was possible. Despite representing only 2.3\% of records, BYD contributed
14.5\% of total prediction error (MAE of 1.735B~VND vs.\ 433M~VND for
non-BYD records). We removed all BYD records from the training dataset.
...
year, price\_vnd, condition, mileage\_km) revealed that 883 records (22.2\%
...
\midrule
Raw (with all issues) & 3,963 & 1.02B VND \\
+ chotot.com price fix & 3,963 & -- \\
+ Deduplication & 3,089 & -- \\
...
battery capacity (kWh), driving range (km), and motor power (hp) for 729
...
\begin{itemize}
    ...
    unique values, $r = 0.76$ with price) and \texttt{model\_year}
...
handle the heavy right-skew of Vietnamese EV prices (skewness = 2.50 for
raw prices, 0.95 for log-transformed). Tree-based models operate on raw
...
prediction errors of approximately 20\% and 10\% of median segment price.
